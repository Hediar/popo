...
│           │                 │                            │
│  ┌────────▼──────┐  ┌───────▼──────────────┐         │
│  │ OpenAIService │  │ ResumeSectionRepository│        │
│  │ (Spring AI)   │  │ (JPA + pgvector)      │        │
│  └───────────────┘  └───────────────────────┘        │
└─────────────────────────────────────────────────────────┘
                          │
┌─────────────────────────────────────────────────────────┐
...
- [x] Next.js 프로젝트 초기화
- [x] 기본 의존성 설정 (Spring AI, JPA, PostgreSQL, Bucket4j, Lombok)
- [x] CORS 설정 (WebConfig)
- [x] IP별 Rate Limiting 설정 (RateLimitFilter, 분당 20회)

### Phase 2: PostgreSQL 셋업 (1일)
- [ ] PostgreSQL 로컬 설정 (popo_ai DB)
- [ ] pgvector 확장 활성화
- [ ] `resume_sections` 테이블 생성
- [ ] 벡터 검색 함수 생성

### Phase 3: Backend - 기본 API (2-3일)
- [ ] OpenAI Service 구현 (Spring AI 활용)
  - 임베딩 생성
  - 채팅 완성 (스트리밍)
- [ ] ResumeSectionRepository 구현
  - 데이터 저장
  - 벡터 검색
...
│   │   │   │   └── RateLimitFilter.java (IP별 Rate Limiting)
│   │   │   └── util/
...
### CORS
- `WebConfig.java`에서 허용 도메인 관리
- `cors.allowed-origins` 프로퍼티로 설정 (배포 시 실제 도메인으로 변경)
- `/api/**` 경로에 GET, POST만 허용

### Rate Limiting
- `RateLimitFilter.java`에서 IP별 요청 제한
- Bucket4j 기반, IP당 분당 20회 제한
- 초과 시 `429 Too Many Requests` 응답
- `X-Forwarded-For` 헤더 지원 (프록시/로드밸런서 대응)

---

## 다음 단계

1. PostgreSQL 로컬 설정 및 pgvector 활성화
2. OpenAI API 키 발급
3. Backend 핵심 서비스 구현 시작
